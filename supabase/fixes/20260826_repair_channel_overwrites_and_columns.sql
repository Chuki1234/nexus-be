-- ============================================================================
-- REPAIR: create_channel_message thất bại 42P01 "channel_overwrites does not exist"
--
-- Triệu chứng: thành viên ĐƯỢC MỜI vào server không gửi được tin trong channel
-- (chủ/owner vẫn gửi được vì đi nhánh admin, không đụng channel_overwrites).
--
-- Hai nguyên nhân chồng nhau, file này sửa cả hai — chạy trong Supabase SQL Editor:
--   1) Bảng public.channel_overwrites bị thiếu trên DB đang chạy.
--   2) Hàm create_channel_message đọc cột allow_permissions/deny_permissions,
--      nhưng schema chuẩn (docs/nexus_schema.sql, migration gốc, hàm 20260824120000)
--      dùng allow/deny. Tên sai do 20260825120000 sinh ra, hotfix mang theo.
--
-- An toàn: idempotent (create ... if not exists), không xoá/đổi dữ liệu sẵn có.
-- ============================================================================

-- 1) Enum target (tạo nếu thiếu) --------------------------------------------
do $$
begin
    if not exists (select 1 from pg_type where typname = 'overwrite_target') then
        create type public.overwrite_target as enum ('role', 'member');
    end if;
end $$;

-- 2) Bảng channel_overwrites (cột allow/deny theo schema đã verify) ----------
...
-- 3) RLS deny-all — khớp 15 bảng còn lại. KHÔNG thêm policy (xem LUẬT CỨNG §3)
alter table public.channel_overwrites enable row level security;
revoke all on public.channel_overwrites from anon, authenticated;

-- 4) Hàm create_channel_message với ĐÚNG tên cột (allow/deny) ----------------
...
begin
    ...
    if not v_is_admin then
        ...
        if v_everyone_role_id is not null then
            select coalesce(allow, 0), coalesce(deny, 0)
        ...
        v_roles_allow := 0;
        v_roles_deny := 0;
        select coalesce(bit_or(coalesce(co.allow, 0)), 0),
               coalesce(bit_or(coalesce(co.deny, 0)), 0)
